<template lang="html">
    <article class="">
        <!-- 第一个items是slider.vue中的items,第二个items是data中的items,此处的意思是将下面data中的items传递给slider.vue中的items
        将$style.slider中的样式传递给slider.vue中的slider样式，重复的部分将其覆盖掉，不重复的部分添加上去 -->
...
<style lang="scss" module>
  ...
  .slider{
    margin-top: 120px;
    // 这里之所以加上width: 100%是因为，当我们调换不同的手机的时候，会出现轮播图中图片的尺寸不能填充整个页面，所有要加上，但是不能在模板组件中加，因为其余的轮播图或许不存在这种问题
    img{
      width: 100%;
    }
  }
  .list{
    ...
    .item{
      text-align: center;
      img{
        // 为了使得图片居中，所以要将img的display设置为inline-block，否则上面的text-align: center;不生效
